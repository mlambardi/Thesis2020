                      ...
                      fixed_totals = NULL, singular.ok = TRUE)
{
    ...
    AS_nc_adjustment <- function(pars, level = 0, fit = NULL) {
        if (is.null(fit)) {
            fit <- key_quantities(pars, y = y, level = level, qr = TRUE)
        }
        with(fit, {
            if (level == 0) {
                ...
                inverse_info_unscaled <- chol2inv(R_matrix)
                b_vector <- numeric(nvars)
                for (j in if (is.null(control$blocks)) seq.int(nvars) else control$blocks) {
                    inverse_info_unscaled_j <- inverse_info_unscaled[, j]
                    vcov_j <- tcrossprod(inverse_info_unscaled_j %*% solve(inverse_info_unscaled[j, j]), inverse_info_unscaled_j)
                    hats_j <- .rowSums((x %*% vcov_j) * x, nobs, nvars, TRUE) * working_weights
                    b_vector[j] <- crossprod(inverse_info_unscaled_j, .colSums(x * (hats_j * (- 0.5 * d2mus/d1mus)), nobs, nvars, TRUE))
                }
                return(.colSums(0.5 * hatvalues * d2mus / d1mus * x, nobs, nvars, TRUE) +
                           info_unscaled %*% b_vector)
            }
            if (level == 1) {
                return(nvars/(2 * dispersion))
            }
        })
    }
